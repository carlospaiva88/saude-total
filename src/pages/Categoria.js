...
const subCategories = {
  ...
  mental: [
    ...
    },
    {
      ...
      name: "Mindfulness",
      description: "Uma prática de atenção plena que ajuda a focar no momento presente.",
...
export default function BlogCategory() {
  ...
  return (
    <>
      ...
      <Breadcrumbs />
      <Container>
        <Title>Explorando Saúde {categoria ? (categoria.charAt(0).toUpperCase() + categoria.slice(1)) : ""}</Title>
        <Subtitle>Escolha um tema para aprofundar seu conhecimento</Subtitle>

        <CategoriesGrid>
          {category.map((sub) => (
            <CategoryLink key={sub.id} to={`/blog/${categoria}/${sub.id}`}>
              <CardBase>
                <CardImage src={sub.image} alt={sub.name} loading="lazy" />
                <CardBody>
                  <CardTitle>{sub.name}</CardTitle>
                  <CardDescription>{sub.description}</CardDescription>
                  <CardButton as={Link} to={`/blog/${categoria}/${sub.id}`}>Ver Artigos</CardButton>
                </CardBody>
              </CardBase>
            </CategoryLink>
          ))}
...
const CategoriesGrid = styled.div`
  ...
  /* força linhas a terem altura igual (combinado com CardBase height:100%) */
  grid-auto-rows: 1fr;

...
export default function TravelHighlights({ travels = {} }) {
  ...
  const renderList = (list) =>
    list.map((t) => {
      ...
      const categoryLabel = (t.category === "nacionais" || t.type === "nacionais")
        ? "Nacional"
        : "Internacional";
        ...
        <Card key={t.slug || t.id} as={CardBase} aria-labelledby={`travel-${t.slug}-title`}>
          ...
          <CardBody>
            ...
            <CardFooter>
              ...
              {/* mostramos label pequena à direita */}
              <CategorySmall>{categoryLabel}</CategorySmall>
            </CardFooter>
          </CardBody>
        </Card>
      );
...
/* label pequena de categoria no rodapé (para quando não quiser sticker) */
const CategorySmall = styled.span`
  font-size: 0.82rem;
  color: ${({ theme }) => theme.colors?.secondaryDark || "#6b6b6b"};
  white-space:nowrap;
`;
